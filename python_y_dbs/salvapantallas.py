...
# Define object turtle as face then specify properties of the object (Object Ordinated Programming)
face = turtle.Turtle()
register_shape("assets/face.gif")
face.shape("assets/face.gif")
face.penup()
...
def moveObject():
    'moveObject will set the initial face image position on the screen then sleeps for 0.01 second. After this it will begin moving the face image positive x and y'
    global foregroundX, foregroundY, foregroundDirX, foregroundDirY, screen
    face.setpos(foregroundX, foregroundY)
    time.sleep(0.01)
...
def changeForegroundXDir():
    ...
    if debug == True:
        ...
        log(DEBUG, "face X = " + str(foregroundX))
        log(DEBUG, "face Y = " + str(foregroundY))
    else:
        log(INFO, "Changing Direction...")
...
def borderCheck():
    'borderCheck checks whether the face image has collided with the side of the screen if so it then runs changeForegroundXDir/changeForegorundYDir and then changeColor'
    global foregroundX, foregroundY, foregroundDirX, foregroundDirY, screen
